# =================================================
# TITAN OMEGA v27.2 – TEMEL ANALİZ ODAKLI SÜRÜM
# =================================================
...
vader = get_vader()
st.set_page_config(page_title="TITAN OMEGA v27.2 – TEMEL ODAKLI", page_icon="🧿", layout="wide")
...
st.markdown("<h1 class='big-title'>TITAN OMEGA v27.2</h1>", unsafe_allow_html=True)
st.markdown("<h2 style='text-align:center; color:#00ffff;'>TEMEL ANALİZ ODAKLI REVİZYON</h2>", unsafe_allow_html=True)
...
def main():
    ...
    if st.button("APEX ANALİZİ BAŞLAT (v27.2)", type="primary", use_container_width=True):
        with st.spinner("TITAN v27.2 Veri Topluyor ve Temel Odaklı Analiz Ediyor..."):
            ...
            # ====================== SKORLAMA MANTIĞI (TEMEL ODAKLI - V27.2) ======================
            score = 50 

            # A. Teknik Skorlama (Ağırlık Azaltıldı - Maks: +33)
            if "GOLDEN" in tech['cross']: score += 15 # Önceki: 20
            elif "BULL TREND" in tech['cross']: score += 8 # Önceki: 10
            elif "DEATH" in tech['cross']: score -= 15 # Önceki: 20
            
            if tech['strength'] == "GÜÇLÜ": score += 8
            if tech['rsi'] < 30: score += 10 
            
            # MACD Skoru (Ağırlık Azaltıldı)
            if "MACD AL" in tech['macd_sig']: score += 10 # Önceki: 12
            elif "MACD SAT" in tech['macd_sig']: score -= 10 # Önceki: 12
            
            # B. Temel Skorlama (Ağırlık Artırıldı - Maks: +54)
            if not fund.get("error"):
                # Adil Değer Potansiyeli
                if fund['upside'] > 30: score += 30 # Önceki: 20
                elif fund['upside'] > 10: score += 15 # Önceki: 10
                elif fund['upside'] < -15: score -= 20 # Önceki: 15 (Negatif etki de arttı)

                # Kalite Metrikleri
                if fund['net_margin'] > 0.05: score += 8 # Önceki: 5
                if fund['debt_to_equity'] < 100: score += 8 # Önceki: 5
                if fund['p_e'] < 20 and fund['p_e'] > 0: score += 8 # Önceki: 5
            ...
            c2.metric("TITAN SKORU (v27.2)", f"{int(score)}/100")
...
if __name__ == "__main__":
    main()
    st.caption("© TITAN OMEGA v27.2 – Temel Odaklı | 2025 | Yatırım tavsiyesi değildir")
